...
PerformDEAnal<-function (anal.type = "default", par1 = NULL, par2 = NULL, nested.opt = "intonly"){
  if (dataSet$de.method == "deseq2") {
    .prepare.deseq(anal.type, par1, par2 , nested.opt);
    .perform.computing();
    res <- .save.deseq.res()
    return(res);
    
  }else{
    .prepareContrast(anal.type, par1, par2, nested.opt);
    res <- .perform_limma_edger()
    return(res)
  }
  
}

.prepare.deseq<-function(anal.type, par1, par2, nested.opt){
  .prepareContrast(anal.type, par1, par2, nested.opt);
  my.fun <- function(){
    print("Peforming DESeq2 Microservice.... ");
    library(DESeq2);
    dataSet <- dat.in$data;
...
.prepareContrast <-function (anal.type = "default", par1 = NULL, par2 = NULL, nested.opt = "intonly"){
  set.seed(1337);
  par1 <<- par1
  par2 <<- par2
...
Volcano.Anal <- function(paired=FALSE, fcthresh, threshp, analType, inx){
  ...
  cat(json.obj);
  sink();
  partialToBeSaved <<- c(partialToBeSaved, c("volcano2.json", "enrichment_result.csv"))
}
