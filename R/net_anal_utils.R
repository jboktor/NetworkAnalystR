...
SearchNetDB <- function(db.type, table.nm, require.exp=TRUE, min.score = 900){
    ...
    if(db.type == "ppi"){
      if(net.type == "innate"){
        # first try to match swissprot, if still have unmatched, do uniprot
        seed.table <- doPpiIDMapping(protein.vec, "entrez_swissprot");
        na.inx <- is.na(seed.table[,1]);
        if(sum(na.inx)>0){
            unmatched.seed <- protein.vec[na.inx];
            seed.table2 <- doPpiIDMapping(unmatched.seed, "entrez_uniprot");
            seed.table <- rbind(seed.table, seed.table2);
        }
...
.prepare.pcsf <- function(){
    ...
    dat.in <- list(data=overall.graph, terminals = expr.vec, my.fun = my.fun);
...
PerformNetEnrichment <- function(file.nm, fun.type, IDs){
    ...
    if(ppi.net$db.type == 'ppi'){
        if(data.org == "ath"){
                ...
                idtype <- "emblprotein"
        }else if(data.org %in% c("hsa","mmu") & net.type %in% c("string","innate", "huri")){
                idtype <- "entrez"
