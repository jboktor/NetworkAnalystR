...
PrepareChordDataInit<-function(){
  ...
  partialToBeSaved <<- c(partialToBeSaved, c(sel.nms))
  venn.genenb <- vector()
  sel.dats <- list();
...
PrepareChordData <-function(){
  ...
  chordData$org = data.org
  chordData$analType=anal.type
  chordData$naviString = "network"
  ...
  partialToBeSaved <<- c(partialToBeSaved, c(fileNm, fileNm2))
  sink(fileNm2);
  cat(toJSON(lookup));
